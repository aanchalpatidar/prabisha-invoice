...
export default function InvoicesPage() {
  ...
  const handleSendEmail = async (invoiceId: string) => {
    try {
      setSending((prev) => ({ ...prev, [invoiceId]: true }))
      ...
      setSending((prev) => ({ ...prev, [invoiceId]: false }))
    }
  }
            ...
            {filteredInvoices.length === 0 ? (
              ...
              filteredInvoices.map((invoice) => (
                <TableRow key={invoice.id} className="hover:bg-muted/50">
                  ...
                  <TableCell><span className={getStatusColor(invoice.status)}>{invoice.status}</span></TableCell>
                  <TableCell>{formatCurrency(invoice.total)}</TableCell>
                  <TableCell className="text-center">
                    <Link href={`/invoices/${invoice.id}`}><Button variant="outline" size="sm"><Eye className="h-4 w-4" /></Button></Link>
                    <Link href={`/invoices/${invoice.id}/edit`}><Button variant="outline" size="sm" className="ml-2"><Edit className="h-4 w-4" /></Button></Link>
                    <LoadingButton variant="outline" size="sm" className="ml-2" onClick={() => handleSendEmail(invoice.id)} loading={!!sending[invoice.id]}><Send className="h-4 w-4" /></LoadingButton>
                    <Button variant="outline" size="sm" className="ml-2" onClick={() => setDeleteDialog({ open: true, invoiceId: invoice.id })}><Trash2 className="h-4 w-4" /></Button>
                  </TableCell>
                </TableRow>
              ))
            )}
